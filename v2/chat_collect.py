...
def _wait_until(hhmmss: str) -> None:
    ...
    ⚠★★**この窓は「放送がHH:00:00ちょうどに始まる」ことに依存している**（2026-09-06）。
      HH:00:50頃の募集アナウンスの直後を拾う設計なので、放送開始がずれると
      **募集する前に締め切る**ことになり、おたよりが1件も拾えなくなる。
      → RadioDJの前半イベントに **`Start Playing` が必須**（曲の途中でも定刻に割り込む）。
        「いま鳴っている曲が終わってから」にすると最大5分ずれて壊れる。
      ⚠ 送出側の設定を変えるときは、ここを一緒に見ること。
        `infra/VPS_SETUP.md` の「前半の Start Playing は入れないと壊れる」に同じ話を書いた。
