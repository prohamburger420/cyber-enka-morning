...
RULES: list[tuple[str, str]] = [
    ...
    # --- 新レーベル「Neoこぶし」（2026-09-18 解禁）---
    # ★英字が入ると英語g2pに流れる。実測 `Neoこぶし` → **Ｎイオコブシ**（ne[oakusuko]bushi）。
    #   CEBDR24 と同じ機序なので、同じくカタカナに畳んで経路ごと回避する。
    (r"Neo\s*こぶし", "ネオコブシ"),
    (r"NEO\s*こぶし", "ネオコブシ"),
    # ★検証ラン(2026-09-18 03:41)で模型が「ネオ・こぶし」「AI歌手」と書いた。本番経路の実測:
    #   ネオ・こぶし → ne[o_ko[bushi（中黒で二語に割れる）／ AI歌手 → a[i]kashu（**アイ歌手**）
    (r"ネオ・こぶし", "ネオこぶし"),
    (r"AI歌手", "エーアイ歌手"),
...
CHECK_WORDS: list[tuple[str, str]] = [
    ...
    ("Neoこぶし", "ネオコブシ"),   # 2026-09-18 解禁の新レーベル。Ｎイオコブシと読んでいた
    ("ネオ・こぶし", "ネオコブシ"),   # 中黒で二語に割れていた
    ("AI歌手の呼魂ひかり", "エーアイカシュ"),   # 「アイ歌手」と読んでいた
