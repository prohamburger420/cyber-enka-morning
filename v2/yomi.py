...
RULES: list[tuple[str, str]] = [
    # --- 英字の局名（2026-09-08 nordw耳判定「ネイティブ英語発音になって不自然。
    #     日本人が読んでる感じに」）---
    # 英字が台本に混じると英語用g2p(g2p_en)が走り、ネイティブ発音で読まれる。
    # カタカナに畳めばこの経路ごと回避できる。「24」は数字のまま残す
    # （pyopenjtalkが「ニジューヨン」と正しく読む。カタカナで書くと逆に不自然になった時に
    #   切り分けられない）。
    (r"CEBDR", "シーイービーディーアール"),
    # --- 一般語の誤読（2026-09-10 v2/yomi_diff.py が検出）---
    # ★「人がいない／いなく」を **ジンガイナイ** と読む（実測）。
    #   「人がいる」「人がいて」は正しく読むので、**「いな」が続く形だけ**が壊れる。
    #   ⚠ 9/10 の放送に乗った文がこれだった（「言ってくれる人がいなくて」）。
    #   ひらがなにすると直る（ヒトガイナクテ）。人名の「人」を壊さないよう、
    #   **「人がいな」の並びだけ**に絞って置換する。
    (r"人がいな", "ひとがいな"),
    # ★「急がば回れ」は辞書側（voice/userdict_entries.csv）で直した。
    #   アクセントを指定できるので、置換表よりそちらが上等（2026-09-10）。
    # --- 助詞「は」がハと読まれる口語形 ---
...
CHECK_WORDS: list[tuple[str, str]] = [
    ("CEBDR24", "シーイービーディーアール"),   # 英字のまま英語g2pに流さない
    ("言ってくれる人がいなくて", "ヒトガイナクテ"),   # ジンガイナクテ と読んでいた（9/10 放送）
    # ⚠「急がば回れ」はここに置けない。**辞書（userdict）で直した**ので、
    #   この検査（素のpyopenjtalk経路）では直らない。回帰は voice/apply_userdict.py 側にある。
    #   ★直し方が2通りあるとき、**検査も同じ側に置く**こと。片方で直して片方で検査すると、
    #     直っているのに落ちる／落ちないのに壊れている、のどちらかになる。
    ("道路ってのはね", "ノワネ"),
    ("あたしの分、話すわね", "ノブン"),
    ("らいおんこずえです", "ライオンコズエ"),
    ("カリカリの縁がうまい", "フチ"),
