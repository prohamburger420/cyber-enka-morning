...
def run(pass_name: str, day: datetime.date, no_audio: bool, traffic_live: bool,
    ...
    if pass_name == "a":
        ...
        # ★★★おたよりが0通なら**台本を作らない**（2026-09-10 実際に事故った）。
        #   空のまま PROMPT_B に渡すと、モデルは「2〜3通を紹介する」という指示に従って
        #   **架空のリスナーを創作する**。実際に 2026-09-10 の6/7/8時、誰も書いていない回で
        #   「みかんさん・てっちゃんさん・くろねこさん」が捏造され、**放送に乗った**
        #   （亡くなった母の話まで作られた）。fake_chat に倒れたのではなく、**純粋な創作**。
        #   ⚠ プロンプトで「無ければ書くな」と頼む形にしない。**コードで止める。**
        #     生成させなければ、パスAが作ったフォールバック版（こずえが自分で答える回）が
        #     そのまま流れる＝番組は成立し、嘘は乗らない。
        if not kept:
            log.warning("★おたよりが0通 → **パスBは何も作らない**"
                        "（フォールバック版おたよりがそのまま流れる）")
            return 0
