...
# ★★★2026-09-18: 16000では足りず、**2日続けて打ち切られてフォールバック台本が放送された**
#   （9/16・9/17。出力ちょうど16000トークンで停止）。
#   ⚠ 原因は台本が長いからではない。**thinkingのトークンが出力に含まれる**。
#     実測 9/15: 台本は1720字しかないのに **出力11522トークン**＝8割がthinking。
#     9/14にCOMMON_RULESへルールを3つ足した（語尾・曲名・募集の前倒し）ぶん
#     考える量が増えて、16000に当たるようになった。
#   ★モデルとeffortは触らない（台本の口調は耳判定で詰めたもの。変えると癖が変わる）。
#     **枠だけ広げる。** ストリーミングで受けているのでタイムアウトの心配はない。
# ★★2026-09-18 再検証: 同じ日の同じ台本で出力が 6571 / 13098 / **19092** トークンとブレた
#   （thinkingの量が回ごとに違う）。32000 の6割まで来る回がある＝運が悪いと当たる。
#   ストリーミングなので枠を広げるコストは無い。**上限に当たる事故だけ**を消す。
MAX_TOKENS = 64000
